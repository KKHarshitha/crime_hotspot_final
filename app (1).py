...
if len(coords) > 0:
    db = DBSCAN(eps=10/6371, min_samples=2, metric="haversine").fit(np.radians(coords))  # 5 km radius
    crime_data["Cluster"] = db.labels_

# Streamlit UI
st.title("📍 Crime Hotspots: Filter Crime Severity Levels")

# Dropdown to filter crime severity
severity_options = ["Low", "Moderate", "High"]
selected_severity = st.selectbox("Select Crime Severity Level:", severity_options)

# Create map
m = folium.Map(location=[16.7107, 81.1004], zoom_start=12)
...
if map_data and "last_clicked" in map_data:
    ...
    for _, row in crime_data.iterrows():
        ...
        if distance_km <= 5 and row["Crime_severity"] == selected_severity:
            nearby_hotspots.append((row["Area Name"], hotspot_lat, hotspot_lon, row["Crime_severity"]))

    if nearby_hotspots:
        st.subheader(f"🚨 {selected_severity}-Severity Crime Hotspots (within 5KM radius)")
        ...
        # Add hotspots based on selected severity
        color_mapping = {"Low": "green", "Moderate": "orange", "High": "red"}
        
        for city, lat, lon, severity in nearby_hotspots:
            folium.CircleMarker(
                location=[lat, lon],
                radius=10,
                color=color_mapping[severity],
                fill=True,
                fill_color=color_mapping[severity],
                fill_opacity=0.7,
                popup=f"{city}: {severity} Severity"
            ).add_to(crime_map)
        
        folium_static(crime_map)
    else:
        st.warning(f"⚠ No {selected_severity}-severity crime hotspots found within 5KM.")
# Main App Logic
def main():
